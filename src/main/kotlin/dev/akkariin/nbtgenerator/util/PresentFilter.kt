...
enum class PresentFilter(val filter: PathFilter?) {
    ZERO(PathFilter { file, _ -> file.name == "zero.json" }),
    DATA_PACKETS(PathFilter { _, tag -> tag?.startsWith("minecraft:datapacks") == false }),
    TAGS(PathFilter { _, tag -> tag?.startsWith("minecraft:tags") == false }),
