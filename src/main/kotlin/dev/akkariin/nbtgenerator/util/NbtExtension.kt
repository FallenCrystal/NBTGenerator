...
@ApiStatus.Internal
inline fun <reified T : Tag> toListTag(type: TagType<T>, collection: Collection<T>) = ListTag.builder(type).also { collection.forEach(it::add) }.build()

fun Collection<CompoundTag>.toListTag() = toListTag(TagTypes.COMPOUND, this)

inline fun <reified T : BinaryTag> T.singletonTag(types: TagType<T>) = toListTag(types, Collections.singleton(this))
